...
class Dsmadmc:
    ...
    def sendline(text):
        if Dsmadmc.analyzer is None or not Dsmadmc.analyzer.isalive():
            ...
            Dsmadmc.analyzer.expect([Dsmadmc.TSM_PROMPT, Dsmadmc.MORE, Dsmadmc.MORE2])
        ...
        Dsmadmc.analyzer.expect([Dsmadmc.TSM_PROMPT, Dsmadmc.MORE, Dsmadmc.MORE2, pexpect.EOF])  ## várjuk, hogy promptot vagy "MORE"-t kapjunk
    ...
    def bye(self):
        ...
        print(colored('Thanks!', 'green'), colored('spadmin has run', 'white'), str(datetime.timedelta(seconds=abs((end_date - dsm.start).seconds))), colored('total','white'))
...
def commandline(prompt):
    ...
    if prompt is not None:
        ...
        text = session.prompt(
        Dsmadmc.TSM_PROMPT, completer=completer, complete_while_typing=False, cursor=CursorShape.BLINKING_BLOCK)  # Prompt, history-val és completerrel.
    dsm.sendline(text)
