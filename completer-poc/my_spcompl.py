...
class IBMSPrlCompleter:
    ...
    def tokenEngine( self, tokens ):
        ...
        if len( tokens )   == 0:
            ...
            logging.info( ' Stepped into LEVEL 1. [' + tokens[ -1 ] )
            ...
            for key in self.rules:
                ...
                if search( key, tokens[ -2 ] ):
                    ...
                    for x in self.rules[key]:
                        if x.startswith( tokens[ -1 ] ):
                            logging.info( ' Starts with [' + x + '>' + tokens[ -1 ] + ']' )    
                            ret.append( x + ' ' )                            
                        elif search( '^(' + tokens[ -1 ], x ):
                            logging.info( ' REGEXP with [' + tokens[ -1 ] + '|' + x + ']' )
            ...
            for key in self.rules:
                if search( key, tokens[ -3 ] + ' ' + tokens[ -2 ]):
                    logging.info( ' FOUND for pattern: II. regexp [' + pformat( self.rules[key] ) + ']' )
                    for x in self.rules[key]:
                        if x.startswith( tokens[ -2 ] ):
                            ret.append( x + ' ' )
                        elif search( tokens[ -2 ], x ):
                            ret.append( search( '^\((\w+)|', x )[1] + ' ' )
    ...
    def IBMSPcompleter( self, text, state ):
        ...
        try:
            ...
            logging.info( 'Results: ' + pformat( self.results, width=180 ) )
            if self.results[ state ] == None:
                logging.info( 'COMPLETER RESULT PUSH CYCLES ENDED! --------------------------------------------------------------------------' )
            else:
                logging.info( 'COMPLETER results push cycle: [' + str( state ) + '] [' + self.results[ state ] + ']. -----------------------------------------------------------' )
            
            return self.results[ state ] 
            #alma = self.results.pop( 0 )
            #logging.info( 'POP: ' + alma + self.results.pop( 0 ) )
            #return 'alma'
